...
import os, fnmatch, ufoProcessor, mojo
...
class TranslationPen(BasePen):
    ...
    def __init__(self, otherPen, frontAngle=0, frontWidth=0):
...
def makeShadow(g, extrusionX, extrusionY):
    ...
    if g.rightMargin%2==0:
        ...
        g.rightMargin+=int(extrusionX-20)
    g.changed()

def makeShadowGlyphBackground(p, g):
    m  = Identity
    m = m.scale(1,1)
    m = tuple(m)
    shadowGlyph = RGlyph()
...
def makeShadowGlyph(p, g, offsetValue):
    ...
    makeShadow(g, extrusionX, extrusionY)
    g.removeOverlap()
    insetPen = g.getPointPen()
...
fontList = listFonts(mastersFolderPath)

for f in fontList:
    if ".ufo" in f:
        f = OpenFont(mastersFolderPath+f)
        for g in f.keys():
            if f[g]:
                extrusionX = -150
                extrusionY = -150
                makeShadowGlyph(f, f[g], 2)
        f.save(f.info.familyName+f.info.styleName+"-shadow.ufo")
print("done!")